...
from model_navigator.pipelines.constants import PIPELINE_MAGICMIND_CONVERSION
...
def magicmind_conversion_builder(config: CommonConfig, models_config: Dict[Format, List[ModelConfig]]) -> Pipeline:
    ...
    """
    # 如果目标芯片不是 MLU，则返回空的管道
    if config.target_device != DeviceKind.MLU:
        return Pipeline(name=PIPELINE_MAGICMIND_CONVERSION, execution_units=[])

    mm_models_config = models_config.get(Format.MAGICMIND, [])
    # run_profiles_search = search_for_optimized_profiles(config, trt_models_config)

    from model_navigator.commands.convert.onnx.onnx2mm import ConvertONNX2MM
    # from model_navigator.commands.copy.copy_model import CopyModelFromPath

    execution_units: List[ExecutionUnit] = []
    for model_cfg in mm_models_config:
        # Convert ONNX to TensorRT again, this time with optimized profiles
        execution_units.append(
            ExecutionUnit(
                command=ConvertONNX2MM,
    ...
    #             )
    #         )
    return Pipeline(name=PIPELINE_MAGICMIND_CONVERSION, execution_units=execution_units)
